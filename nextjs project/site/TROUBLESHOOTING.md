...
Run the diagnostic script to check common issues:
```bash
npm run diagnose
```
...
**Cause**: Usually a runtime error in getStaticProps/getServerSideProps
**Fix**: Check server component data fetching, add try-catch blocks

### "Error: Image Optimization"
**Cause**: Missing or incorrect image configurations
**Fix**: Ensure all images in public/images have correct formats

### "Module parse failed: Unexpected token"
**Cause**: Syntax error or unsupported JavaScript feature
**Fix**: Check for ES6+ syntax not transpiled, update babel/webpack config

### "ECONNREFUSED" or "ETIMEDOUT"
**Cause**: External API connection issues during build
**Fix**: Add timeout handling, implement fallback data
...
# On Vercel
# Trigger redeployment with cleared cache
```
...
### 3. Bypass Sanity Temporarily
Set environment variable:
```
NEXT_PUBLIC_USE_FALLBACK_DATA=true
```

### 4. Debug Build Locally with Vercel CLI
```bash
npm i -g vercel
vercel build
vercel dev
